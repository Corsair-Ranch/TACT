...
const requestCostSummaries = async (req, res) => {
    ...
    .then((data) => {
      if (data[0].roleID === roleAdmin) {
        ...
        res
          .status(200)
          .json(
            `Error: Unable to determine user role -- \"${data[0].roleID}\"`
          );
      }
    });
    ...
    case dropdownOptionByFY:
      if (role === roleAdmin) {
        ...
        const exerciseData = await knex("exercises")
          ...
          .whereBetween("exerciseStartDate", [
            `10-01-${value - 1}`,
            `09-30-${value}`,
          ]);
  ...
  exerciseaircraftData.map((airframeData) => {
    ...
    const lodging =
      Number(airframeData.commercialLodgingCost) +
      Number(airframeData.governmentLodgingCost);

    const meals =
      Number(airframeData.mealPerDiem) *
      Number(airframeData.mealNotProvidedCount) *
      travelDuration;
        ...
        knex("exercises")
          ...
          .then((data) => {
            ...
            data.map((row) => {
              ...
              if (
                dateFormatted.getMonth() === 10 ||
                dateFormatted.getMonth() === 11 ||
                dateFormatted.getMonth() === 12
              ) {
