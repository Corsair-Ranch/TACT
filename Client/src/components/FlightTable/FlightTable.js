...
class simpleflightoffer {
  constructor(offer) {
    this.departureStartAirport =
      offer.itineraries[0].segments[0].departure.iataCode;
    this.departureStartTime = new Date(
      offer.itineraries[0].segments[0].departure.at
    )
      .toTimeString()
      .slice(0, 5);
    this.departureEndAirport =
      offer.itineraries[0].segments.slice(-1)[0].arrival.iataCode;
    this.departureEndTime = new Date(
            ...
            {simple_data.map((offer) => (
              ...
              >
                <TableCell component="th" scope="offer">
                  <Button
                    variant="contained"
                    onClick={() => selectButtonClick(offer.cost)}
                  >
                    Select
                  </Button>
                </TableCell>
                <TableCell align="right">
                  {offer.departureStartAirport}: {offer.departureStartTime}
                  <br />
                  {offer.departureEndAirport}: {offer.departureEndTime}
                </TableCell>
                <TableCell align="right">{offer.departureStopovers}</TableCell>
                <TableCell align="right">
                  {offer.returnStartAirport}: {offer.returnStartTime}
                  <br />
                  {offer.returnEndAirport}: {offer.returnEndTime}
                </TableCell>
